...
import os
if "bateau_3_0.1" not in os.listdir("./RealTimeMOT/YoloV8") :
  os.mkdir("./RealTimeMOT/YoloV8/bateau_3_0.1")
  os.mkdir("./RealTimeMOT/YoloV8/bateau_3_0.1/det")
  os.mkdir("./RealTimeMOT/YoloV8/bateau_3_0.1/img1")

if "output" not in os.listdir("./RealTimeMOT/YoloV8") :
  os.mkdir("./RealTimeMOT/YoloV8/output")

det = open("./RealTimeMOT/YoloV8/bateau_3_0.1/det/det.txt","w")
det.close()

from ultralytics import YOLO
model = YOLO("./RealTimeMOT/YoloV8/models/yolov8n.pt") 
...
CONF_TRESHOLD = 0.10
OBJECTS_OF_INTEREST = ["boat"]


## Création de seqinfo.ini
vs = cv2.VideoCapture("./inputVideos/bateau/bateau_3.mp4")

if not vs.isOpened() :
  raise SystemError("Couldn't read the input video")

seq = open("./RealTimeMOT/YoloV8/bateau_3_0.1/seqinfo.ini","w")
seq.write("[Sequence]\nname=bateau_3_0.1\nimDir=img1")
seq.close()
...
try:
	...
	seq = open("./RealTimeMOT/YoloV8/bateau_3_0.1/seqinfo.ini","a")
...
while acc < total :
  ...
  if grabbed:
    ...
    cv2.imwrite("./RealTimeMOT/YoloV8/bateau_3_0.1/img1/"+zeros[:i-len(str(acc))]+str(acc)+".jpg", frame)
    ...
    for box in boxes:
      ...
      if box[-2] > CONF_TRESHOLD and label in OBJECTS_OF_INTEREST :
          color = COLORS[int(box[-1])]
          box_label(frame, box, label, color)

          # save detection in det.txt
          det = open("./RealTimeMOT/YoloV8/bateau_3_0.1/det/det.txt","a")
    ...
    if writer is None:
      ...
      writer = cv2.VideoWriter('./RealTimeMOT/YoloV8/output/detections_bateau_3_0.1.avi', fourcc=fourcc, fps=fps,
